...
| [`/pitch-deck-audit`](./pitch-deck-audit/) | Claude Code skill + standalone playbook | Audit a fundraise deck (or structurer proposal, M&A teaser) using a sector-tuned lens. Ships SKILL.md + a standalone playbook + a fillable lens template + a Cowork (no-install) workflow guide. Five worked sector lenses (credit-fintech / regulated structured-product / hard-tech / M&A teaser / structurer-to-counterparty) land in a follow-up release. **`v0.1.0-beta` — versioned beta, narrow distribution.** |
